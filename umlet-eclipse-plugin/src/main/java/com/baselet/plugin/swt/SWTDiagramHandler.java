...
public class SWTDiagramHandler extends com.baselet.diagram.DiagramHandler {

	private static final Logger log = LoggerFactory.getLogger(SWTDiagramHandler.class);
	...
	public static SWTDiagramHandler forExport(FontHandler fontHandler) {
		SWTDiagramHandler returnHandler = new SWTDiagramHandler(null, false);
		if (fontHandler != null) {
	...
	public SWTDiagramHandler(File diagram) {
		this(diagram, false);
	}

	protected SWTDiagramHandler(File diagram, boolean nolistener) {
		super(diagram, nolistener);
	...
	private void reloadPalettes() {
		for (SWTPaletteHandler d : SWTMain.getInstance().getPalettes().values()) {
	...
	public void doClose() {
		if (askSaveIfDirty()) {
			...
			SWTDiagramHandler newhandler = (SWTDiagramHandler) CurrentDiagram.getInstance().getDiagramHandler();
			if (newhandler != null) {
	...
	public boolean askSaveIfDirty() {
		if (isChanged) {
			MessageBox messageBox = new MessageBox(null, SWT.ICON_QUESTION | SWT.YES | SWT.CANCEL | SWT.NO);
			messageBox.setText("Save diagram");
			messageBox.setMessage("Save modified diagram?");
			...
			return true;
			// if (MessageDialog.openConfirm(null, "Save Diagram", "Save modified diagram?")) {
			// doSave();
			// }
